  ...
  const handleForceSync = async () => {
    ...
    try {
      ...
      if (res.successCount > 0) {
        ...
        setSyncFeedback('Outbox is already fully up to date with cloud!');
      }
    } catch (err: any) {
      ...
      case 'failed':
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-bold text-rose-700 bg-rose-50 border border-rose-200 px-2 py-0.5 rounded-full font-mono">
            <AlertCircle className="w-3 h-3 text-rose-600" /> Failed (Will Retry)
          </span>
        );
      default:
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-bold text-amber-700 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-full font-mono">
            <Clock className="w-3 h-3 text-amber-600" /> Queued (Waiting for online)
          </span>
        );
    }
  };
          ...
          {items.length === 0 ? (
            ...
            items.map((item) => (
              ...
              >
                ...
                {item.status === 'failed' && (
                  <div className="mt-2.5 pt-2 border-t border-rose-100 flex justify-end">
                    <button
                      type="button"
        ...
        <div className="p-4 border-t border-slate-150 bg-slate-50 space-y-2">
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>{isManualSyncing ? 'Pushover to Firebase...' : 'Force Sync Outbox Now'}</span>
            </button>
          ...
          <p className="text-[10px] text-slate-400 text-center font-medium">
            Vernunt background outbox keeps your family data safe and resilient through intermittent networks.
          </p>
        </div>
      </div>
    </div>
